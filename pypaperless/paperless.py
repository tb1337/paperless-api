...
class PaperlessAPI:
    ...
    def __init__(
        ...
        self.auth = Auth(
            endpoint=endpoint, token=token, username=username, password=password
        )

    async def __get_paginated_results(
        self, path: str, isa: object, params=None
    ) -> List[object]:
        ...
        async with ClientSession() as session:
            while True:
                ...
                # extend list
                results.extend([isa(data, self.auth) for data in json["results"]])
    ...
    async def __get_non_paginated_results(
        self, path: str, isa: object, params=None
    ) -> List[object]:
        """Walk through the pagination and return a list of objects of type <isa>."""
        json = {}
        ...
        async with ClientSession() as session:
            ...
            # extend list
            results.extend([isa(data, self.auth) for data in json])
        return results
    ...
    async def add_correspondent(self, data: dict) -> Correspondent:
        """Create correspondent."""
        json = {}
        async with ClientSession() as session:
            resp = await self.auth.request(session, "post", "correspondents", data=data)
            resp.raise_for_status()
            json = await resp.json()
        return json
